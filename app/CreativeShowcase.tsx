...
export default function CurvyShowcase() {
  return (
    <main className="text-[#1a1a1a] min-h-screen px-6 py-20 lg:px-24 font-zalando">
      {/* ── HEADER ── */}
      <header className="max-w-4xl mb-24 relative z-10">
        ...
        >
          Focusing on the{" "}
          <span className="italic font-light">intersection</span> of technical
      ...
      {/* ── Wave Divider ── */}
      <div
        className="absolute left-0 right-0 h-[300px] pointer-events-none opacity-[0.03]"
        style={{
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-y-32 gap-x-16 relative z-10">
        {SHOWCASE_ITEMS.map((item, i) => (
          ...
          >
            ...
            <div
              className="group relative aspect-[14/10] overflow-hidden mb-10"
              style={{
                position: "relative",
                maskSize: "100% 100%",
...
/>
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between items-start pt-2">
              <div className="flex gap-6 items-center mb-4 lg:mb-0">
                <span className="text-[9px] font-mono opacity-20 border border-black/10 px-2 py-0.5 rounded-full">
                  {item.id}
                </span>
